...
Tests all PlayGodot native debugger features. Requires a custom Godot build
with automation support.

Run with:
    export GODOT_PATH=/path/to/godot-automation-fork
    pytest tests/test_playgodot_demo.py -v
"""
...
async def test_query_nodes(game):
    ...
    assert len(cells) >= 9  # At least 9 cells in tic-tac-toe
